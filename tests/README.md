...
sides of BOT-RELAY's surface area:

- the local BOT-RELAY receiver (`/health`, `/api/srebot/{stats,latest,events}`)
...
optionally `BOT_RELAY_RECEIVER_URL` from `.env`. Make sure the local BOT-RELAY
...
- `test_srebot_latest.json` returns 404 (`"No SREBOT payloads received
...
`tests/test_api_routes.py` 是这个仓库里唯一的测试。它会同时验证 BOT-RELAY
的两个面：

- 本地 BOT-RELAY 接收器（`/health`、`/api/srebot/{stats,latest,events}`）
...
以及可选的 `BOT_RELAY_RECEIVER_URL`。先确认本地 BOT-RELAY 接收器在跑（`python
main.py`），否则本地接收器的探测会超时而不是返回 200。

### 预期内的部分失败

- 如果还没有任何 envelope 通过 `/ws/srebot` 推过来，
  `test_srebot_latest.json` 会得到 404
...
- SREBOT 部署完成之后，确认每个封装过的接口仍然返回 BOT-RELAY 预期的结构。
