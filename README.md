# AXBot — the CN-branch bot (SREBOT toolkit)

## English

**AXBot is the CN-branch bot's repo, but today it contains no bot code.**
No Discord, no command handlers, no scheduler, no business logic — just
the plumbing. The bot side gets built on top of this foundation: this
repo gives you everything you need to talk to SREBOT and turn its data
into whatever you want (text, JSON, scoreboard PNGs, leaderboards,
...
3. Create a `.env` file at the repo root:
   ```
   AXBOT_RECEIVER_HOST=0.0.0.0
   AXBOT_RECEIVER_PORT=18081
   SREBOT_API_BASE_URL=http://your-srebot-host:18081
   SREBOT_API_BEARER_TOKEN=your-shared-bearer-token
   ```

   - `AXBOT_RECEIVER_HOST` / `AXBOT_RECEIVER_PORT` — where AXBot binds
     its HTTP API. `0.0.0.0:18081` is the default.
   - `SREBOT_API_BASE_URL` — base URL of the remote SREBOT bridge. Used
     for both HTTP queries and (by deriving `ws://`) the WebSocket
     connection to SREBOT's push endpoint.
   - `SREBOT_API_BEARER_TOKEN` — the shared bearer used for **both**
   ...
   - `SREBOT_WS_URL` — (optional) explicit WebSocket URL override. If not
     set, AXBot derives it from `SREBOT_API_BASE_URL` by replacing
     `http://` → `ws://` (or `https://` → `wss://`) and appending
     `/ws/srebot`. Only set this if the WS endpoint lives at a different
...
5. Start the receiver:
   ...
   `http://${AXBOT_RECEIVER_HOST}:${AXBOT_RECEIVER_PORT}`. Bot code you
   add to this repo can pull from `/api/srebot/{stats,latest,events}` or
...
3. `listen_forever()` connects to SREBOT's external bridge at
   `ws://<SREBOT_API_BASE_URL>/ws/srebot` (or `SREBOT_WS_URL` if set)
...
5. Bot code in this repo reads pushed envelopes from
   `/api/srebot/latest` (newest) or `/api/srebot/events` (short history)
...
| `GET`  | `/api/srebot/stats`                        | In-memory envelope counts grouped by `event_type` |
| `GET`  | `/api/srebot/latest?event_type=`           | Newest envelope (404 if store empty)              |
| `GET`  | `/api/srebot/events?event_type=&limit=100` | Short history, newest first                       |
...
- Scoreboard art lookup order:
  1. `AXBOT_SCOREBOARD_ASSETS_DIR` (optional env override)
...
3. 在仓库根目录创建 `.env`：
   ...
   - `AXBOT_RECEIVER_HOST` / `AXBOT_RECEIVER_PORT` —— AXBot HTTP API 的
     绑定地址，默认 `0.0.0.0:18081`。
   - `SREBOT_API_BASE_URL` —— 远端 SREBOT bridge 的基址。同时用于 HTTP
     查询，以及（通过替换协议头）推导 WebSocket 连接地址。
   - `SREBOT_API_BEARER_TOKEN` —— 共用 bearer token，**同时**用于 AXBot
     对外 HTTP 查询 SREBOT 和对外 WebSocket 连接。一个 token 覆盖两个
     方向。
   - `SREBOT_WS_URL` —— （可选）显式指定 WebSocket URL。不设置时 AXBot
     会把 `SREBOT_API_BASE_URL` 的 `http://` 换成 `ws://`（`https://`
     换成 `wss://`）再加上 `/ws/srebot`。只有 WS 端点地址和 HTTP API
     不同时才需要手动设置。
...
5. 启动接收器：
   ...
   envelope。HTTP API 在
   `http://${AXBOT_RECEIVER_HOST}:${AXBOT_RECEIVER_PORT}` 上可用。之后
   加在这个仓库里的 bot 代码可以拉
   `/api/srebot/{stats,latest,events}`，或者调用
...
3. `listen_forever()` 连接到 SREBOT 外部 bridge 的
   `ws://<SREBOT_API_BASE_URL>/ws/srebot`（或 `SREBOT_WS_URL`），开始
   接收推送的 envelope。
4. SREBOT 每处理完一场比赛就广播一条 envelope，以 zstd 压缩的二进制帧
   ...
   30 秒上限。
5. 仓库内的 bot 代码通过 `/api/srebot/latest`（最新）或
   `/api/srebot/events`（简短历史）读取推送过来的 envelope ——
...
|---|---|---|
| `GET` | `/health` | 健康检查 |
| `GET` | `/api/srebot/stats` | 按 `event_type` 分组的内存 envelope 计数 |
| `GET` | `/api/srebot/latest?event_type=` | 最新一条 envelope（如果还没有则 404） |
| `GET` | `/api/srebot/events?event_type=&limit=100` | 简短历史，按时间倒序 |
...
- `src/assets/DAGOR_FILES/` 下的 Dagor/VROMFS 解析器依赖 `zstandard` 和
  ...
  可以解析。
- Scoreboard 美术资源查找顺序：
  1. `AXBOT_SCOREBOARD_ASSETS_DIR`（可选 env 覆盖）
